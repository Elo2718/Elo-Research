...
# Walk-Forward CV and Save Final Model from Last Fold
def walk_forward_cv(X, y, config, train_size=10000, val_size=1000, n_splits=7):
    ...
    for i in tqdm(range(n_splits)):
        ...
        train_loader = DataLoader(TensorDataset(X_train_tcn, y_train_tcn), batch_size=config['batch_size'], shuffle=True)
    ...
    # Save final model
    torch.save(final_model, "model.pth")
    print("Final model saved as model.pth")

    return avg_score

# Grid Search (Single Best Config)
param_space = {
